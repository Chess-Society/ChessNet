...
const GET = async ({ url, cookies }) => {
  ...
  const { PUBLIC_SUPABASE_URL, PUBLIC_SUPABASE_ANON_KEY } = await import("../../../../chunks/public.js");
  ...
  try {
    const classId = url.searchParams.get("class_id");
    const studentId = url.searchParams.get("student_id");
    let query = supabase.from("class_students").select("*").eq("owner_id", user.id);
    if (classId) {
...
const POST = async ({ request, url, cookies }) => {
  ...
  try {
    ...
    const { data: classData, error: classError } = await supabase.from("classes").select("id, user_id").eq("id", class_id).eq("user_id", user.id).single();
    ...
    const { data: studentData, error: studentError } = await supabase.from("students").select("id, user_id").eq("id", student_id).eq("user_id", user.id).single();
    if (studentError || !studentData) {
      console.error("❌ API /api/class-students POST - Student not found or not owned by user:", studentError);
      return json({ error: "Student not found" }, { status: 404 });
    }
    const { data: existingEnrollment, error: existingError } = await supabase.from("class_students").select("id").eq("class_id", class_id).eq("student_id", student_id).eq("owner_id", user.id).single();
    if (existingEnrollment) {
      return json({ error: "Student is already enrolled in this class" }, { status: 409 });
    }
    const { data: newEnrollment, error: insertError } = await supabase.from("class_students").insert({
      class_id,
      student_id,
      owner_id: user.id,
      enrolled_at: (/* @__PURE__ */ new Date()).toISOString()
