  ...
  if (isLocalDev) {
    ...
    const mockClassStudents = [
      ...
      // Estudiantes en mock-class-2 (Intermedios Tarde)
      {
        ...
        enrolled_at: new Date(Date.now() - 15 * 24 * 60 * 60 * 1e3).toISOString(),
        status: "active",
      ...
      },
      {
        ...
        enrolled_at: new Date(Date.now() - 10 * 24 * 60 * 60 * 1e3).toISOString(),
        status: "active",
    ...
    const enrolledStudents = mockClassStudents.filter((cs) => cs.class_id === classId && cs.status === "active").map((cs) => cs.student);
    ...
    const attendanceStats = enrolledStudents.map((student) => {
      const studentRecords = recentAttendance.flatMap(
        (session) => session.records.filter((record) => record.student_id === student.id)
      );
      ...
      return {
        ...
        last_attendance_date: studentRecords.length > 0 ? recentAttendance[recentAttendance.length - 1].date : void 0
      };
    });
    return {
      user: locals.user,
