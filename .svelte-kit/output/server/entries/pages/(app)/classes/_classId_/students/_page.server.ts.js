...
const load = async ({ locals, url, params, cookies }) => {
  ...
  if (isLocalDev) {
    ...
    const enrolledStudents = mockStudents.filter((s) => enrolledStudentIds.includes(s.id)).map((student) => ({
      ...student,
      enrolled_at: classEnrollments.find((e) => e.student_id === student.id)?.enrolled_at
    }));
    const availableStudents = mockStudents.filter(
      (s) => s.active && !enrolledStudentIds.includes(s.id)
    );
    const stats = {
  ...
  try {
    ...
    const { data: availableStudents, error: studentsError } = await supabase.from("students").select("*").eq("user_id", locals.user.id).eq("active", true).not("id", "in", `(${enrolledStudentIds.join(",")})`);
    if (studentsError) {
