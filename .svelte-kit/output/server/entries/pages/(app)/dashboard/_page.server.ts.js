  ...
  if (isLocalDev) {
    ...
    try {
      const [schoolsResult, studentsResult, classesResult] = await Promise.all([
        supabase2.from("colleges").select("id, name, city, created_at").eq("user_id", locals.user.id),
        supabase2.from("students").select("id").eq("user_id", locals.user.id),
        supabase2.from("classes").select("id").eq("user_id", locals.user.id)
      ]);
      const dashboardStats = {
        ...
        // Asumimos que todos están activos por ahora
        monthlyRevenue: 0,
        // Se calculará cuando implementemos el sistema de pagos
        upcomingSessions: 0
        // Se calculará cuando implementemos el sistema de clases
      };
      const centersWithStats = (schoolsResult.data || []).map((school) => ({
        ...
        totalClasses: 0,
        // Se calculará cuando tengamos clases
        totalStudents: 0,
        // Se calculará cuando tengamos estudiantes
        occupancyRate: 0,
        attendanceRate: 0,
        monthlyRevenue: 0,
