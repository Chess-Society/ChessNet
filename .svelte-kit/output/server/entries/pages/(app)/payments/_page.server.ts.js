...
const load = async ({ locals, url }) => {
  ...
  if (isLocalDev) {
    ...
    const { mockPayments, mockStudents, mockSchools } = await import("../../../../chunks/mockData.js");
    ...
    const paymentStats = {
      total_revenue_this_month: enrichedPayments.filter((p) => p.status === "paid").reduce((sum, p) => sum + p.amount, 0),
      total_revenue_last_month: 980,
      // Mock del mes pasado
      ...
      overdue_amount: enrichedPayments.filter((p) => p.status === "overdue").reduce((sum, p) => sum + p.amount, 0),
      average_payment_amount: enrichedPayments.length > 0 ? enrichedPayments.reduce((sum, p) => sum + p.amount, 0) / enrichedPayments.length : 0
    };
    const revenue_growth_percentage = 15;
    const studentSummaries = mockStudents.map((student) => {
      ...
      return {
        ...
        last_payment_date: (/* @__PURE__ */ new Date()).toISOString(),
        next_due_date: (/* @__PURE__ */ new Date()).toISOString()
      };
    }).filter((s) => s.total_payments > 0);
    const schoolSummaries = mockSchools.map((school) => {
      const schoolPayments = enrichedPayments.filter((p) => p.school_id === school.id || p.student && mockStudents.find((s) => s.id === p.student_id)?.college_id === school.id);
      return {
        school_id: school.id,
  ...
  try {
    ...
    return {
      ...
      paymentStats: stats,
      studentSummaries: [],
      // Implementar si es necesario para la UI
      schoolSummaries: []
    };
  } catch (err) {
