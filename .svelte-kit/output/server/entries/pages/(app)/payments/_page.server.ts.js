const load = async ({ locals, url }) => {
  ...
  if (isLocalDev) {
    ...
    const paymentStats = {
      total_revenue_this_month: mockPayments.filter((p) => p.status === "paid" && new Date(p.paid_date || p.created_at).getMonth() === (/* @__PURE__ */ new Date()).getMonth()).reduce((sum, p) => sum + p.amount, 0),
      ...
      overdue_amount: mockPayments.filter((p) => p.status === "overdue").reduce((sum, p) => sum + p.amount, 0),
      average_payment_amount: mockPayments.length > 0 ? mockPayments.reduce((sum, p) => sum + p.amount, 0) / mockPayments.length : 0
    };
    const revenue_growth_percentage = paymentStats.total_revenue_last_month > 0 ? Math.round((paymentStats.total_revenue_this_month - paymentStats.total_revenue_last_month) / paymentStats.total_revenue_last_month * 100) : 0;
