...
const load = async ({ locals, url, params, cookies }) => {
  ...
  if (isLocalDev) {
    ...
    const mockClasses = [
      ...
      },
      {
        ...
        created_at: new Date(Date.now() - 120 * 24 * 60 * 60 * 1e3).toISOString(),
        updated_at: new Date(Date.now() - 14 * 24 * 60 * 60 * 1e3).toISOString()
      },
      // Clases de mock-school-2 (Club Ajedrez Infantil Retiro)
      {
        ...
        updated_at: new Date(Date.now() - 7 * 24 * 60 * 60 * 1e3).toISOString()
      },
      {
        id: "mock-class-5",
        user_id: "dev-user-123",
  ...
  try {
    const { data: school, error: schoolError } = await supabase.from("colleges").select("*").eq("id", params.schoolId).eq("user_id", locals.user.id).single();
    if (schoolError) {
    ...
    const { data: classes, error: classesError } = await supabase.from("classes").select("*").eq("college_id", school.id).eq("user_id", locals.user.id).order("created_at", { ascending: false });
    if (classesError) {
    ...
    const stats = {
      totalClasses: schoolClasses.length,
      activeClasses: schoolClasses.length,
      // Todas las clases están activas por defecto
      inactiveClasses: 0,
      // No hay campo active en la tabla
      totalStudents: 0,
      // No hay campo current_students en la tabla
      totalCapacity: schoolClasses.reduce((sum, c) => sum + (c.max_students || 0), 0),
      occupancyRate: 0,
      // No se puede calcular sin current_students
      levels: {
        ...
        mixed: 0
        // No hay nivel mixed en el esquema
      },
      averageClassSize: 0
      // No se puede calcular sin current_students
    };
