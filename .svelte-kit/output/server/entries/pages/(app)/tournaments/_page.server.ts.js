...
const load = async ({ locals, url, cookies }) => {
  ...
  if (isLocalDev) {
    ...
    const tournamentStats = {
      ...
      average_players_per_tournament: mockTournaments.length > 0 ? mockTournaments.reduce((sum, t) => sum + t.players_registered, 0) / mockTournaments.length : 0
    };
  ...
  try {
    const { data: tournaments, error: tournamentsError } = await supabase.from("tournaments").select("*").eq("user_id", locals.user.id).order("created_at", { ascending: false });
    if (tournamentsError) {
    ...
    const { data: availableStudents, error: studentsError } = await supabase.from("students").select("id, first_name, last_name, chess_level, school_id").eq("user_id", locals.user.id).eq("active", true);
    if (studentsError) {
