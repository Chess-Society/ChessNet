...
export const load = async ({ locals, url, params, cookies }: Parameters<PageServerLoad>[0]) => {
  ...
  if (isLocalDev) {
    ...
    // Skills disponibles como prerequisitos (excluyendo la actual)
  ...
  try {
    ...
    // Obtener habilidades disponibles como prerequisitos (excluyendo la actual)
    const { data: availablePrerequisites, error: prerequisitesError } = await supabase
      .from('skills')
      .select('id, name, difficulty')
      .eq('user_id', locals.user.id)
      .neq('id', params.skillId)
      .eq('active', true)
      .order('difficulty');
