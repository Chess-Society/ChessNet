const load = async ({ locals, url }) => {
  ...
  if (isLocalDev) {
    ...
    const { mockStudents, mockSchools, mockClasses, mockPayments } = await import("../../../../chunks/mockData.js");
    const mockStudentsReports = mockStudents.slice(0, 10).map((student) => {
      ...
      return {
        ...
        progress_summary: {
          ...
          days_enrolled: Math.floor((Date.now() - new Date(student.created_at).getTime()) / (1e3 * 60 * 60 * 24)),
          total_sessions: 20,
          ...
          payment_compliance: studentPayments.length > 0 ? studentPayments.filter((p) => p.status === "paid").length / studentPayments.length * 100 : 100,
          ...
          last_activity_date: (/* @__PURE__ */ new Date()).toISOString().split("T")[0]
        },
        recent_activity: [
          {
    ...
    const generalStats = {
      ...
      active_students: mockStudentsReports.filter(
        (s) => new Date(s.progress_summary.last_activity_date) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1e3)
      ).length,
