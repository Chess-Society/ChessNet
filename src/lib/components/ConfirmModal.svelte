    ...
    >
      ...
      <div class="p-6">
        <div class="flex items-start gap-4">
          <div class="flex-shrink-0 p-3 rounded-xl border {getTypeClasses($uiStore.confirmDialog.type)}">
            <svelte:component this={getIcon($uiStore.confirmDialog.type)} weight="fill" size={24} />
          </div>
        ...
        <div class="mt-8 flex flex-col sm:flex-row-reverse gap-3">
          <button
            type="button"
            class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white shadow-lg transition-all active:scale-95 {getButtonClasses($uiStore.confirmDialog.type)}"
            on:click={() => $uiStore.confirmDialog?.onConfirm()}
          >
            <Check weight="bold" size={16} />
            {$uiStore.confirmDialog.confirmText || 'Confirmar'}
          </button>
          ...
          <button
            ...
            on:click={() => $uiStore.confirmDialog?.onCancel()}
          >
