<script lang="ts">
  ...
  import { Warning, Info, Trash, Check, X, Keyboard } from 'phosphor-svelte';
...
{#if $uiStore.promptDialog}
  ...
  >
    ...
    <div 
      ...
      onclick={() => uiStore.closePrompt()}
      onkeydown={(e) => e.key === 'Escape' && uiStore.closePrompt()}
      role="button"
      tabindex="-1"
    ></div>
    ...
    {#if $uiStore.promptDialog}
      ...
      <!-- Assuming prompts are usually warning or danger level -->
      {@const styles = getTypeStyles('danger')}
      {@const Icon = getIcon('danger')}
      <div 
          ...
          <div class="flex flex-col items-center text-center gap-8">
            ...
            <div class="w-full relative z-10">
              <input 
                ...
                class="w-full bg-black/50 border border-red-500/30 px-6 py-4 text-sm font-mono font-black uppercase tracking-widest text-red-400 placeholder:text-red-900/50 focus:border-red-500 outline-none transition-all text-center"
                onkeydown={(e) => {
                  if (e.key === 'Enter') {
