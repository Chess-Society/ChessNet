...
<div class="fixed inset-0 z-[100] flex items-center justify-center p-4 lg:p-8" in:fade={{ duration: 200 }}>
  ...
  {#if student}
    <div 
      class="relative w-full max-w-2xl bg-zinc-900 border border-zinc-800 rounded-[40px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
    ...
    >
      ...
      <div class="p-8 lg:p-10 bg-zinc-950/50 border-b border-zinc-800 flex items-start justify-between relative overflow-hidden">
        <div class="absolute top-0 right-0 w-64 h-64 bg-violet-500/5 blur-[80px] rounded-full -mr-32 -mt-32"></div>
        
        <div class="flex items-center gap-6 relative z-10">
          <div class="w-20 h-20 bg-violet-600 rounded-3xl flex items-center justify-center text-white font-outfit font-black text-2xl shadow-2xl shadow-violet-500/20 uppercase tracking-tighter">
            {getInitials(student.name)}
          </div>
          <div>
            <div class="flex items-center gap-3 mb-2">
              <span class="px-3 py-1 bg-violet-500/10 border border-violet-500/20 rounded-full text-[10px] font-black text-violet-400 uppercase tracking-widest">{$t('students.student_label')}</span>
        ...
        <button 
          onclick={onclose}
          class="w-12 h-12 bg-zinc-900 border border-zinc-800 rounded-2xl flex items-center justify-center text-zinc-500 hover:text-white hover:border-zinc-700 transition-all active:scale-95 relative z-10"
        >
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar p-8 lg:p-10">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <!-- Attendance Card -->
          <div class="bg-zinc-950/50 border border-zinc-800 rounded-3xl p-6 relative overflow-hidden group">
            ...
            <div class="grid grid-cols-3 gap-3">
              <div class="bg-zinc-900/50 rounded-2xl p-3 text-center border border-zinc-800/50">
                <p class="text-[10px] font-black text-zinc-600 uppercase mb-1">P</p>
                <p class="text-lg font-outfit font-black text-emerald-400">{attendanceStats.p}</p>
              </div>
              <div class="bg-zinc-900/50 rounded-2xl p-3 text-center border border-zinc-800/50">
          ...
          <div class="bg-zinc-950/50 border border-zinc-800 rounded-3xl p-6 relative overflow-hidden group">
            <div class="flex items-center justify-between mb-6">
              ...
              <span class="px-2 py-1 {paymentStatus === 'ok' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border-rose-500/20'} border rounded-lg text-[8px] font-black uppercase tracking-widest">
          ...
          <!-- Competition Card -->
          <div class="bg-zinc-950/50 border border-zinc-800 rounded-3xl p-6 md:col-span-2 relative overflow-hidden group">
            <div class="flex items-center gap-3 mb-6">
              <div class="w-10 h-10 bg-fuchsia-500/10 border border-fuchsia-500/20 rounded-xl flex items-center justify-center text-fuchsia-400">
            ...
            {#if tournamentResults.length > 0}
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {#each tournamentResults.slice(0, 4) as result}
                  <div class="bg-zinc-900/50 border border-zinc-800/50 rounded-2xl p-4 flex items-center justify-between">
                    <div>
                    ...
                    <div class="flex items-center gap-2">
                       <span class="w-8 h-8 rounded-lg bg-zinc-950 flex items-center justify-center text-sm font-outfit font-black text-amber-500">
                        {result.points || 0}
                       </span>
                    </div>
                  </div>
      ...
      <div class="p-8 bg-zinc-950/80 backdrop-blur-md border-t border-zinc-800 flex items-center justify-between">
        ...
        >
          <div class="w-10 h-10 bg-zinc-900 border border-zinc-800 rounded-xl flex items-center justify-center group-hover:bg-zinc-800 transition-all">
        ...
        <button 
          onclick={onclose}
          class="h-12 px-8 bg-zinc-900 border border-zinc-800 text-white font-black rounded-xl hover:bg-zinc-800 transition-all active:scale-95 uppercase text-[10px] tracking-widest"
        >
          {$t('common.close')}
        </button>
      </div>
