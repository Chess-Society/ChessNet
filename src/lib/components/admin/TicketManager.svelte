...
<div class="space-y-6 sm:space-y-8">
  ...
  <div class="grid grid-cols-1 gap-4 sm:gap-6">
    {#each sortedTickets as ticket (ticket.id)}
      <div 
        class="bg-zinc-900/40 backdrop-blur-xl border border-white/5 p-5 sm:p-8 rounded-none shadow-2xl relative overflow-hidden group"
        in:slide
      >
          ...
          <div class="flex-1 space-y-6">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-4">
                <div class="w-12 h-12 {ticket.status === 'open' ? 'bg-amber-500/10 text-amber-500' : 'bg-violet-500/10 text-violet-500'} rounded-none flex items-center justify-center border border-white/5">
                ...
                <div>
                  <div class="flex items-center gap-3">
                    <span class="text-[10px] font-black uppercase tracking-widest {ticket.status === 'open' ? 'text-amber-500' : 'text-violet-500'}">
              ...
              <div class="flex items-center gap-2">
                ...
                <button 
                  ...
                  class="p-3 bg-red-500/10 text-red-500 rounded-none border border-red-500/20 hover:bg-red-500 hover:text-white transition-all"
                >
            ...
            <p class="text-xs sm:text-sm text-slate-400 leading-relaxed font-medium bg-black/20 p-4 sm:p-6 rounded-none border border-white/5">
              {ticket.content}
            </p>

            {#if ticket.adminResponse}
              <div class="p-6 bg-primary-500/5 border border-primary-500/10 rounded-none space-y-2">
          ...
          {#if ticket.status === 'open'}
            <div class="xl:w-80 space-y-4" in:fade>
              ...
              <textarea 
                ...
                class="w-full bg-black/40 border border-white/10 rounded-none p-4 text-xs font-medium text-white outline-none focus:border-primary-500 transition-all resize-none min-h-[120px]"
              ></textarea>
              <button 
                onclick={() => handleRespond(ticket.id)}
                disabled={isSending[ticket.id] || !responseText[ticket.id]}
                class="w-full py-4 bg-primary-500 text-white rounded-none text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-3 hover:translate-y-[-2px] active:translate-y-[0] transition-all disabled:opacity-50 shadow-lg shadow-primary-500/20"
              >
        ...
        <div class="absolute -right-20 -bottom-20 w-40 h-40 {ticket.status === 'open' ? 'bg-amber-500/10' : 'bg-violet-500/10'} blur-[100px] rounded-none"></div>
      </div>
    {:else}
      <div class="py-20 text-center border-2 border-dashed border-white/5 rounded-none space-y-6">
        <div class="w-20 h-20 bg-white/5 rounded-none flex items-center justify-center mx-auto text-slate-700">
