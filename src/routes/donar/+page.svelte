<script lang="ts">
  import { Heart, School } from 'lucide-svelte';
  import { fade } from 'svelte/transition';
</script>

<svelte:head>
  <title>Apoya el Desarrollo | ChessNet</title>
  <meta name="description" content="Ayuda a mantener ChessNet gratuito y en constante mejora. Tu donación nos permite seguir desarrollando nuevas funcionalidades para profesores de ajedrez." />
</svelte:head>
...
<div class="min-h-screen bg-[#0f172a] text-white font-sans selection:bg-emerald-500/30 overflow-x-hidden">
  ...
  <!-- Content -->
  <main class="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center" in:fade={{ duration: 800 }}>
    <div>
      <div class="inline-flex items-center justify-center p-3 rounded-2xl bg-pink-500/10 text-pink-500 mb-6 border border-pink-500/20 shadow-lg shadow-pink-500/10">
        <Heart class="w-8 h-8 fill-current" />
      </div>
      <h1 class="text-4xl md:text-5xl font-extrabold tracking-tight text-white mb-6">Apoya el desarrollo</h1>
      <p class="text-lg text-slate-400 leading-relaxed mb-12 max-w-2xl mx-auto">
        ChessNet es un proyecto joven creado con pasión por el ajedrez.
        Tu contribución nos ayuda a mantener los servidores y seguir
        desarrollando nuevas funcionalidades gratuitas para la
        comunidad.
      </p>

      <div class="bg-slate-900/50 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl inline-block max-w-md w-full">
      ...
      <p class="mt-8 text-sm text-slate-500">
        ...
        Gracias por ser parte de ChessNet.
      </p>
    </div>
  </main>
</div>
