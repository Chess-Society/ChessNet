...
<div class="p-4 sm:p-8 max-w-7xl mx-auto space-y-16 pb-24" in:fade>
  ...
  <header 
    class="relative overflow-hidden rounded-none p-8 sm:p-14 border border-white/10 bg-gradient-to-br from-indigo-950/40 via-slate-900/20 to-black backdrop-blur-2xl shadow-2xl"
    in:fly={{ y: -30, duration: 1000 }}
  >
    <div class="absolute -top-32 -right-32 w-80 h-80 bg-violet-600/20 blur-[120px] rounded-none animate-pulse"></div>
    <div class="absolute -bottom-32 -left-32 w-80 h-80 bg-blue-600/10 blur-[120px] rounded-none"></div>

    <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-10">
      <div class="space-y-6">
        <div class="flex items-center gap-3">
          <div class="p-3.5 rounded-none bg-violet-500/10 border border-violet-500/20 text-violet-400">
      ...
      <div class="flex flex-col items-center lg:items-end gap-6">
        <div class="relative group">
          ...
          <div class="relative bg-white/5 border border-white/10 backdrop-blur-xl p-8 rounded-none flex items-center gap-10 shadow-2xl">
  ...
  <div class="flex flex-wrap items-center justify-between gap-6 pb-4 border-b border-white/5">
    <div class="flex items-center gap-2 p-1.5 bg-white/5 border border-white/10 rounded-none backdrop-blur-md">
      {#each ['all', 'unlocked', 'locked'] as f}
        <button 
          class="px-6 py-2.5 rounded-none text-[10px] font-black transition-all uppercase tracking-widest {filter === f ? 'bg-white text-black shadow-xl' : 'text-slate-400 hover:text-white'}"
          onclick={() => filter = f}
        >
    ...
    <button 
      ...
      class="flex items-center gap-3 px-6 py-3 rounded-none bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 font-bold text-[10px] uppercase tracking-widest hover:bg-indigo-500/20 transition-all"
    >
  ...
  <div class="space-y-24">
        ...
        {#if catInsignias.length > 0}
            <section class="space-y-10" id="section-{cat.id}">
                <div class="flex items-center gap-6 group/section">
                    <div class="p-4 rounded-none bg-white/5 border border-white/10 text-white/40 shadow-inner group-hover/section:text-violet-400 group-hover/section:border-violet-500/30 transition-all duration-500">
                    ...
                    <div>
                        <h2 class="text-3xl font-black text-white tracking-tight uppercase italic flex items-center gap-4">
                            ...
                            <span class="h-0.5 w-12 bg-white/10 rounded-none group-hover/section:w-24 group-hover/section:bg-violet-600 transition-all duration-700"></span>
                        </h2>
                    </div>
                </div>
    ...
    {#if filteredInsignias.length === 0}
        <div class="py-32 text-center space-y-6 bg-white/5 rounded-none border border-dashed border-white/10" in:fade>
            <div class="inline-flex p-8 rounded-none bg-slate-900 border border-white/5 text-slate-700">
  ...
  <!-- Bottom CTA / Legend -->
  <section 
    class="mt-32 relative p-12 overflow-hidden rounded-none text-center border border-white/10 bg-white/5 backdrop-blur-md"
    in:fly={{ y: 50, duration: 1000 }}
  >
    <div class="absolute inset-0 bg-gradient-to-b from-indigo-600/5 to-transparent"></div>
    <div class="relative z-10 space-y-8">
      <div class="inline-flex p-5 rounded-none bg-indigo-500/10 text-indigo-400">
      ...
      <button 
        onclick={() => goto('/panel')} 
        class="group flex items-center mx-auto gap-3 bg-white text-black py-5 px-12 rounded-none font-black text-[10px] uppercase tracking-widest transition-all hover:scale-105 active:scale-95 shadow-2xl"
      >
