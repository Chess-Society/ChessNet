...
<div class="max-w-7xl mx-auto px-6 pb-20" transition:fade>
    ...
    <div class="space-y-4">
      <div class="flex items-center gap-6">
        ...
        <div>
          ...
          <p class="text-slate-400 font-jakarta text-lg font-medium tracking-tight mt-1">Organiza tu academia y gestiona horarios de entrenamiento.</p>
        </div>
      </div>
    </div>
  ...
  {#if classes.length === 0}
    ...
    <div class="bento-card border-dashed border-white/10 p-20 md:p-32 text-center space-y-10 relative overflow-hidden" in:fade>
      ...
      <button 
        onclick={() => goto(!hasSchools ? '/panel/centros' : '/panel/clases/create')}
  ...
  {:else if filteredClasses.length === 0}
    <div class="bento-card border-dashed border-white/10 p-24 text-center space-y-8 relative overflow-hidden">
      ...
      <div class="space-y-3 relative z-10">
        ...
        <p class="text-slate-500 font-jakarta text-base font-medium">No encontramos clases que coincidan con "<span class="text-violet-400">{searchQuery}</span>".</p>
      </div>
      <button 
        onclick={() => searchQuery = ''}
