...
<div class="max-w-[1600px] mx-auto px-6 pb-24" in:fade>
  ...
  <div class="fixed inset-0 pointer-events-none overflow-hidden z-0">
    ...
    <div class="absolute bottom-[5%] left-[-10%] w-[35rem] h-[35rem] bg-blue-500/5 rounded-full blur-[100px] animate-pulse" style="animation-delay: 2s"></div>
  </div>
  ...
  <div class="relative z-10 flex flex-col lg:flex-row lg:items-end justify-between gap-12 mb-20 pt-16">
    ...
    <div class="flex items-center gap-4 bg-zinc-900/50 p-3 rounded-[2.5rem] border border-white/5 backdrop-blur-2xl shadow-2xl">
      <div class="flex items-center bg-black/40 rounded-2xl p-1.5 border border-white/5 mr-4">
        <button 
          onclick={() => viewMode = 'list'}
          class="px-4 py-2.5 rounded-xl transition-all {viewMode === 'list' ? 'bg-zinc-800 text-primary-400 shadow-lg' : 'text-slate-500 hover:text-slate-300'}"
        >
          <Selection size={18} weight="bold" />
        </button>
        <button 
          onclick={() => viewMode = 'grid'}
          class="px-4 py-2.5 rounded-xl transition-all {viewMode === 'grid' ? 'bg-zinc-800 text-primary-400 shadow-lg' : 'text-slate-500 hover:text-slate-300'}"
        >
      ...
      <button 
        ...
        class="bg-white text-black px-12 py-4.5 rounded-[1.75rem] text-[10px] font-black uppercase tracking-[0.3em] hover:bg-primary-600 hover:text-white transition-all shadow-2xl flex items-center gap-4 active:scale-95 font-outfit group overflow-hidden relative"
      >
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-20 relative z-10">
    <div class="bento-card group p-10 bg-zinc-900/40 backdrop-blur-3xl border border-white/5 rounded-[3rem] relative overflow-hidden transition-all duration-700 hover:border-primary-500/30">
      ...
      <div class="relative z-10 flex flex-col gap-10">
        <div class="w-16 h-16 rounded-[1.5rem] bg-primary-500/10 border border-primary-500/20 flex items-center justify-center text-primary-400 group-hover:shadow-glow-primary transition-shadow">
        ...
        <div>
          ...
          <div class="flex items-end gap-3 mt-4">
            ...
            <div class="px-2 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-[10px] font-black text-emerald-400 mb-2 uppercase tracking-tight">{$t('classes.status.active')}</div>
          </div>
        </div>
      </div>
    </div>

    <div class="bento-card group p-10 bg-zinc-900/40 backdrop-blur-3xl border border-white/5 rounded-[3rem] relative overflow-hidden transition-all duration-700 hover:border-blue-500/30">
      <div class="absolute -right-8 -bottom-8 opacity-[0.03] group-hover:opacity-[0.08] transition-all duration-700 group-hover:scale-125 group-hover:-rotate-12">
        <UsersThree size={240} weight="fill" />
      </div>
      <div class="relative z-10 flex flex-col gap-10">
        <div class="w-16 h-16 rounded-[1.5rem] bg-blue-500/10 border border-blue-500/20 flex items-center justify-center text-blue-400 group-hover:shadow-glow-blue transition-shadow">
    ...
    <div class="bento-card group p-10 bg-zinc-900/40 backdrop-blur-3xl border border-white/5 rounded-[3rem] relative overflow-hidden transition-all duration-700 hover:border-amber-500/30">
      <div class="absolute -right-8 -bottom-8 opacity-[0.03] group-hover:opacity-[0.08] transition-all duration-700 group-hover:scale-125 group-hover:-rotate-12">
        <Target size={240} weight="fill" />
      </div>
      <div class="relative z-10 flex flex-col gap-10 h-full">
        <div class="w-16 h-16 rounded-[1.5rem] bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-amber-400 group-hover:shadow-glow-amber transition-shadow">
        ...
        <div>
          ...
          <div class="flex items-center gap-5 mt-4">
            ...
            <div class="flex-1 h-3 bg-zinc-950 rounded-full border border-white/5 p-0.5 overflow-hidden shadow-inner">
              <div class="h-full bg-gradient-to-r from-amber-600 to-amber-400 rounded-full transition-all duration-1000" style="width: {metrics.avgOccupancy}%"></div>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="bento-card group p-10 bg-zinc-900/40 backdrop-blur-3xl border border-white/5 rounded-[3rem] relative overflow-hidden transition-all duration-700 hover:border-emerald-500/30">
      <div class="absolute -right-8 -bottom-8 opacity-[0.03] group-hover:opacity-[0.08] transition-all duration-700 group-hover:scale-125 group-hover:-rotate-12">
        <CurrencyEur size={240} weight="fill" />
      </div>
      <div class="relative z-10 flex flex-col gap-10">
        <div class="w-16 h-16 rounded-[1.5rem] bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 group-hover:shadow-glow-emerald transition-shadow">
        ...
        <div>
          ...
          <div class="flex items-end gap-3 mt-4">
            ...
            <div class="flex items-center gap-1 text-emerald-400 text-[10px] font-black mb-1.5 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/10">
  ...
  <div class="relative z-10 flex flex-col xl:flex-row gap-8 mb-12 items-stretch xl:items-center justify-between">
    <div class="flex items-center gap-3 bg-zinc-900/50 p-2.5 rounded-[2.25rem] border border-white/5 backdrop-blur-2xl shadow-xl overflow-x-auto no-scrollbar">
       {#each ['all', 'active', 'empty'] as tab}
         <button 
           onclick={() => currentTab = tab}
           class="px-10 py-4.5 rounded-[1.5rem] text-[10px] font-black uppercase tracking-[0.3em] transition-all whitespace-nowrap {currentTab === tab ? 'bg-white text-black shadow-2xl scale-105' : 'text-slate-500 hover:text-white hover:bg-white/5'}"
         >
    ...
    <div class="flex items-center gap-6 flex-1 xl:max-w-3xl">
      <div class="relative group flex-1">
        <div class="absolute -inset-1 bg-primary-500/5 rounded-[2rem] blur-xl opacity-0 group-focus-within:opacity-100 transition-opacity"></div>
        ...
        <input
          ...
          class="w-full bg-zinc-900/50 border border-white/5 rounded-[2rem] pl-16 pr-8 py-5.5 text-base text-white focus:border-primary-500/50 focus:bg-zinc-900/80 outline-none transition-all placeholder:text-zinc-700 font-jakarta font-semibold shadow-inner"
        />
        {#if searchQuery}
          <button 
            onclick={() => searchQuery = ''}
      ...
      <button class="w-16 h-16 bg-zinc-900/50 border border-white/5 rounded-2xl flex items-center justify-center text-slate-400 hover:text-primary-400 hover:border-primary-500/30 transition-all active:scale-95 shadow-2xl relative overflow-hidden group">
  ...
  <!-- Content Section -->
  {#if classes.length === 0}
    <div class="relative z-10 bento-card border-dashed border-white/10 p-32 text-center space-y-12 rounded-[4rem] group" in:fade>
        <div class="absolute inset-x-0 top-0 h-64 bg-gradient-to-b from-primary-600/5 to-transparent"></div>
        <div class="w-48 h-48 bg-zinc-950 rounded-[4rem] flex items-center justify-center mx-auto border border-white/10 text-slate-700 shadow-3xl relative z-10 group-hover:rotate-12 transition-all duration-1000">
        ...
        <button 
            onclick={() => goto('/panel/classes/create')}
            class="bg-white text-black px-20 py-7 rounded-[2.5rem] font-outfit font-black text-[12px] uppercase tracking-[0.4em] hover:bg-primary-600 hover:text-white transition-all shadow-3xl active:scale-95 relative z-10"
        >
            {$t('classes.empty_btn')}
        </button>
    </div>
  {:else if filteredClasses.length === 0}
    <div class="relative z-10 bento-card border-dashed border-white/10 p-32 text-center space-y-12 rounded-[4rem]" in:fade>
      <div class="w-48 h-48 bg-zinc-950 rounded-[4rem] flex items-center justify-center mx-auto border border-white/10 text-slate-800 shadow-2xl relative z-10 rotate-(-12deg)">
  ...
  {:else}
    {#if viewMode === 'list'}
      <div class="relative z-10 bento-card bg-zinc-900/30 backdrop-blur-3xl rounded-[3.5rem] border border-white/5 overflow-hidden shadow-3xl" in:fade>
                ...
                >
                  <td class="px-12 py-12">
                    <div class="flex items-center gap-10">
                      <div class="relative group/art">
                        <div class="absolute -inset-2 bg-primary-500/20 rounded-[1.75rem] blur-xl opacity-0 group-hover/row:opacity-100 transition-opacity duration-700"></div>
                        <div class="w-20 h-20 bg-zinc-950 border border-white/5 rounded-[1.75rem] flex items-center justify-center text-white font-outfit font-black text-3xl shadow-2xl relative z-10 overflow-hidden group-hover/row:border-primary-500/30 transition-colors">
                      ...
                      <div>
                        ...
                        <div class="flex items-center gap-4 mt-5">
                          <span class="px-3 py-1 bg-white/5 border border-white/5 rounded-lg text-[10px] font-black text-slate-400 uppercase tracking-widest">{cls.level || $t('classes.level.mixed')}</span>
                          {#if (cls.studentIds?.length || 0) >= (cls.max_students || 10)}
                            <span class="px-3 py-1 bg-red-500/10 border border-red-500/20 rounded-lg text-[10px] font-black text-red-400 uppercase tracking-widest animate-pulse">{$t('classes.metrics.full_capacity')}</span>
                          {/if}
                        </div>
                      </div>
                    </div>
                  ...
                  <td class="px-12 py-12">
                     <div class="flex items-center gap-4 group/chip">
                        <div class="p-3 bg-blue-500/10 rounded-xl border border-blue-500/20 text-blue-400 group-hover/row:scale-110 transition-transform">
                  ...
                  <td class="px-12 py-12">
                    <div class="flex items-center gap-4 group/chip">
                       <div class="p-3 bg-amber-500/10 rounded-xl border border-amber-500/20 text-amber-400 group-hover/row:scale-110 transition-transform">
                  ...
                  <td class="px-12 py-12">
                    <div class="flex flex-col gap-4 w-64">
                       ...
                       <div class="h-2.5 bg-black/40 rounded-full border border-white/5 p-0.5 overflow-hidden">
                          <div 
                            class="h-full bg-gradient-to-r from-primary-600 via-emerald-500 to-teal-400 rounded-full transition-all duration-1500 ease-out shadow-glow-primary/20"
                  ...
                  <td class="px-12 py-12 text-right" onclick={(e) => e.stopPropagation()}>
                    <div class="flex items-center justify-end gap-5">
                      <button 
                        onclick={() => goto(`/panel/classes/${cls.id}`)}
                        class="w-14 h-14 rounded-2xl bg-zinc-950/80 border border-white/10 flex items-center justify-center text-slate-500 hover:text-primary-400 hover:border-primary-500/30 transition-all active:scale-95 group/btn"
                      >
                         <Eye size={24} weight="duotone" class="group-hover/btn:scale-110 transition-transform" />
                      </button>
                      <button 
                        onclick={() => goto(`/panel/classes/${cls.id}/edit`)}
                        class="w-14 h-14 rounded-2xl bg-zinc-950/80 border border-white/10 flex items-center justify-center text-slate-500 hover:text-blue-400 hover:border-blue-500/30 transition-all active:scale-95 group/btn"
                      >
                         <PencilSimple size={24} weight="duotone" class="group-hover/btn:scale-110 transition-transform" />
                      </button>
                      <button 
                        onclick={() => deleteClass(cls.id)}
                        class="w-14 h-14 rounded-2xl bg-zinc-950/80 border border-white/10 flex items-center justify-center text-slate-500 hover:text-red-400 hover:border-red-500/30 hover:bg-red-500/5 transition-all active:scale-95 group/btn"
                      >
    ...
    {:else}
      <div class="relative z-10 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10" in:fade>
        {#each filteredClasses as cls, i}
           <div 
             class="group bento-card p-10 bg-zinc-900/40 backdrop-blur-3xl border border-white/5 rounded-[3.5rem] relative overflow-hidden transition-all duration-700 hover:border-primary-500/30 cursor-pointer"
             onclick={() => goto(`/panel/classes/${cls.id}`)}
           ...
           >
              <div class="absolute -right-16 -top-16 w-64 h-64 bg-primary-600/5 rounded-full blur-[80px] group-hover:bg-primary-600/10 transition-colors"></div>
              
              <div class="relative z-10 flex flex-col gap-10 h-full">
                 <div class="flex items-start justify-between">
                    <div class="w-24 h-24 bg-zinc-950 border border-white/10 rounded-[2rem] flex items-center justify-center text-white font-outfit font-black text-4xl shadow-2xl relative overflow-hidden group-hover:scale-110 transition-transform duration-700">
                    ...
                    <div class="flex flex-col items-end gap-3">
                       <span class="px-4 py-1.5 bg-primary-500/10 border border-primary-500/20 rounded-full text-[9px] font-black text-primary-400 uppercase tracking-widest">{cls.level || $t('classes.level.beginner')}</span>
                    </div>
                 </div>
                    ...
                    <div class="space-y-3">
                       ...
                       <div class="flex items-center gap-3">
                          ...
                          <div class="flex-1 h-2 bg-black/40 rounded-full overflow-hidden">
                             <div class="h-full bg-primary-500 rounded-full" style="width: {Math.min(100, (cls.studentIds?.length || 0) / (cls.max_students || 10) * 100)}%"></div>
                          </div>
                       </div>
                    </div>
                    <div class="space-y-3 text-right">
                       <p class="text-[9px] font-black text-slate-600 uppercase tracking-[0.2em]">{$t('classes.metrics.monthly_fee')}</p>
                       <p class="text-2xl font-outfit font-black text-emerald-400 tracking-tighter">{cls.price || 0}<span class="text-sm ml-1 text-slate-600">{$t('common.currency_symbol')}</span></p>
                    </div>
                 </div>
                    ...
                    <div class="flex items-center gap-4">
                       <div class="flex -space-x-3">
                          {#each Array(Math.min(3, cls.studentIds?.length || 0)) as _}
                            <div class="w-10 h-10 rounded-xl bg-zinc-800 border-2 border-zinc-950 flex items-center justify-center text-[10px] font-black text-slate-500">
                               <Users size={16} />
                            </div>
                          {/each}
                          {#if (cls.studentIds?.length || 0) > 3}
                            <div class="w-10 h-10 rounded-xl bg-primary-600 border-2 border-zinc-950 flex items-center justify-center text-[10px] font-black text-white">
