...
<div class="space-y-10 animate-fade-in pb-20" in:fade>
  ...
  <div class="flex flex-col md:flex-row md:items-end justify-between gap-8">
    <div class="space-y-6">
      ...
      <div class="flex items-center gap-6">
        <div class="w-16 h-16 bg-primary-500/10 border border-primary-500/20 rounded-none flex items-center justify-center text-primary-400 shadow-2xl shadow-primary-500/20">
    ...
    <!-- View Switcher -->
    <div class="flex items-center gap-1 bg-zinc-900/50 p-1.5 rounded-none border border-zinc-800 backdrop-blur-xl">
      <button 
        onclick={() => currentView = 'take'}
        class={`px-6 py-2.5 rounded-none text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${currentView === 'take' ? 'bg-white text-black shadow-xl shadow-white/5' : 'text-zinc-500 hover:text-white'}`}
      >
      ...
      <button 
        ...
        class={`px-6 py-2.5 rounded-none text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${currentView === 'calendar' ? 'bg-white text-black shadow-xl shadow-white/5' : 'text-zinc-500 hover:text-white'}`}
      >
      ...
      <button 
        ...
        class={`px-6 py-2.5 rounded-none text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${currentView === 'history' ? 'bg-white text-black shadow-xl shadow-white/5' : 'text-zinc-500 hover:text-white'}`}
      >
      ...
      <button 
        ...
        class={`px-6 py-2.5 rounded-none text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${currentView === 'stats' ? 'bg-white text-black shadow-xl shadow-white/5' : 'text-zinc-500 hover:text-white'}`}
      >
  ...
  {#if currentView === 'take'}
    <div class="space-y-8" in:fly={{ y: 20 }}>
      <!-- Control Bar Card -->
      <div class="bg-zinc-900/50 border border-zinc-800 rounded-none p-6 lg:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-2xl relative overflow-hidden group">
        <div class="absolute top-0 right-0 w-64 h-64 bg-primary-500/5 blur-[100px] -mr-32 -mt-32"></div>
        
        <div class="flex items-center gap-6 relative z-10">
          <div class="p-4 bg-zinc-950 rounded-none border border-zinc-800 shadow-inner group-hover:border-primary-500/30 transition-colors">
        ...
        <div class="flex items-center gap-3 relative z-10">
          <button 
            ...
            class="bg-zinc-950 text-white px-6 py-4 rounded-none text-[10px] font-black uppercase tracking-[0.1em] border border-zinc-800 hover:border-primary-500/50 transition-all flex items-center gap-2 hover:shadow-[0_0_30px_-10px_rgba(139,92,246,0.2)]"
          >
          ...
          <button 
            ...
            class="bg-primary-500 text-black px-10 py-4 rounded-none text-[10px] font-black uppercase tracking-[0.2em] hover:bg-white transition-all shadow-[0_10px_40px_-10px_rgba(139,92,246,0.3)] flex items-center gap-3 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {#if isSubmitting}
              <div class="animate-spin rounded-none h-4 w-4 border-2 border-black border-t-transparent"></div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {#each students as student}
          <div class="bg-zinc-900/50 border border-zinc-800 p-6 rounded-none flex items-center justify-between group hover:border-zinc-700 transition-all shadow-xl hover:shadow-2xl">
            <div class="flex items-center gap-5">
              <div class="w-16 h-16 bg-zinc-950 rounded-none border border-zinc-800 flex items-center justify-center text-primary-400 font-black text-2xl group-hover:border-primary-500/50 transition-colors shadow-inner">
            ...
            <div class="flex bg-zinc-950 p-2 rounded-none border border-zinc-800 gap-1.5 shadow-inner">
              {#each Object.entries(statusThemes) as [status, theme]}
                <button 
                  onclick={() => currentAttendance[student.id].status = status as AttendanceStatus}
                  class={`w-12 h-12 rounded-none flex items-center justify-center transition-all ${currentAttendance[student.id]?.status === status ? `${theme.bg} ${theme.color} ${theme.border} shadow-lg scale-105` : 'text-zinc-700 hover:text-zinc-400 hover:bg-zinc-900'}`}
                  title={theme.label}
                >
  ...
  {:else if currentView === 'calendar'}
    <div class="space-y-8" in:fade>
      <div class="bg-zinc-900/50 border border-zinc-800 rounded-none p-8 lg:p-12 shadow-2xl relative overflow-hidden group">
        ...
        <div class="flex items-center justify-between mb-12 relative z-10">
          <div class="flex items-center gap-6">
            <div class="w-14 h-14 bg-zinc-950 rounded-none border border-zinc-800 flex items-center justify-center text-primary-400 shadow-inner">
          ...
          <div class="flex items-center gap-2">
            <button 
              ...
              class="p-4 bg-zinc-950 border border-zinc-800 rounded-none text-zinc-500 hover:text-white hover:border-primary-500/50 transition-all shadow-inner active:scale-95"
            >
              <CaretLeft weight="bold" class="w-4 h-4" />
            </button>
            <button 
              onclick={() => currentMonth = new Date()}
              class="px-6 py-4 bg-zinc-950 border border-zinc-800 rounded-none text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-white hover:border-primary-500/50 transition-all shadow-inner active:scale-95"
            >
        ...
        <div class="mt-12 pt-8 border-t border-zinc-800/50 flex items-center gap-8 relative z-10">
          <div class="flex items-center gap-3">
            <div class="w-3 h-3 bg-primary-500 rounded-none"></div>
            <span class="text-[9px] font-black text-zinc-500 uppercase tracking-widest">{$t('attendance.calendar.recorded')}</span>
          </div>
          <div class="flex items-center gap-3">
             <div class="w-3 h-3 bg-zinc-950 border border-zinc-800 rounded-none"></div>
  ...
  {:else if currentView === 'history'}
    <div class="space-y-6" in:fly={{ y: 20 }}>
      {#if recentAttendance.length === 0}
        <div class="bg-zinc-900/50 border border-zinc-800 rounded-none p-24 text-center space-y-6">
           <div class="w-24 h-24 bg-zinc-950 rounded-none border border-zinc-800 flex items-center justify-center mx-auto text-zinc-800">
      ...
      {:else}
        <div class="grid grid-cols-1 gap-4">
          {#each recentAttendance as session}
            <div class="bg-zinc-900/50 border border-zinc-800 p-6 rounded-none group hover:border-primary-500/30 transition-all shadow-xl">
               <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div class="flex items-center gap-6">
                     <div class="w-14 h-14 bg-zinc-950 border border-zinc-800 rounded-none flex items-center justify-center text-primary-400 shadow-inner group-hover:border-primary-500/30 transition-colors">
                     ...
                     <div>
                        ...
                        <div class="flex gap-4 mt-3">
                           ...
                           <div class="flex items-center gap-1.5">
                             <span class="w-1.5 h-1.5 rounded-none bg-yellow-500"></span>
                           ...
                           <div class="flex items-center gap-1.5">
                             <span class="w-1.5 h-1.5 rounded-none bg-red-500"></span>
                  ...
                  <button 
                    ...
                    class={`px-8 py-3.5 rounded-none text-[10px] font-black uppercase tracking-[0.15em] border transition-all ${showingDate === session.date ? 'bg-white text-black border-white shadow-xl shadow-white/5' : 'bg-zinc-950 text-white border-zinc-800 hover:border-primary-500'}`}
                  >
  ...
  {:else if currentView === 'stats'}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" in:fly={{ y: 20 }}>
      {#each attendanceStats as stats}
        <div class="bg-zinc-900/50 border border-zinc-800 p-8 rounded-none space-y-8 relative overflow-hidden group hover:border-zinc-700 transition-all shadow-xl">
