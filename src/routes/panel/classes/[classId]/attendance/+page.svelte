<script lang="ts">
  ...
  const statusThemes = {
    P: { icon: CheckCircle, label: 'PRESENT', color: 'text-primary-400', bg: 'bg-primary-500/10', border: 'border-primary-500/20' },
    T: { icon: Clock, label: 'LATE', color: 'text-yellow-400', bg: 'bg-yellow-500/10', border: 'border-yellow-500/20' },
    A: { icon: XCircle, label: 'ABSENT', color: 'text-red-400', bg: 'bg-red-500/10', border: 'border-red-500/20' }
  };
</script>
  ...
  <div class="flex flex-col md:flex-row md:items-end justify-between gap-8">
    <div class="space-y-6">
      ...
      <div class="flex items-center gap-6">
        ...
        <div>
          <h1 class="text-4xl font-black text-white tracking-tighter uppercase leading-none">Attendance</h1>
    ...
    <div class="space-y-8" in:fly={{ y: 20 }}>
      ...
      <div class="bg-zinc-900/50 border border-zinc-800 rounded-[24px] p-6 lg:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-2xl relative overflow-hidden group">
        ...
        <div class="flex items-center gap-6 relative z-10">
          ...
          <div>
            <p class="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-1.5">Session Date</p>
            <input 
              type="date" 
            ...
            <div class="flex items-center gap-5">
              ...
              <div>
                ...
                <div class="flex items-center gap-2 mt-2">
                  <span class="text-[9px] font-black text-zinc-500 uppercase tracking-widest">{student.age} years</span>
    ...
    <div class="space-y-6" in:fly={{ y: 20 }}>
      {#if recentAttendance.length === 0}
        <div class="bg-zinc-900/50 border border-zinc-800 rounded-[32px] p-24 text-center space-y-6">
           ...
           <div>
             <h3 class="text-white font-black uppercase tracking-widest text-sm">No previous records</h3>
             <p class="text-zinc-500 text-[10px] font-medium uppercase mt-2 tracking-widest">Saved records will appear here</p>
           </div>
        </div>
      {:else}
          ...
          {#each recentAttendance as session}
            <div class="bg-zinc-900/50 border border-zinc-800 p-6 rounded-[24px] group hover:border-primary-500/30 transition-all shadow-xl">
               <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div class="flex items-center gap-6">
                     ...
                     <div>
                        <h4 class="text-white font-black uppercase tracking-tighter text-xl leading-none">
                          {new Date(session.date).toLocaleDateString('en-US', { weekday: 'long', day: 'numeric', month: 'long' })}
                        </h4>
                        <div class="flex gap-4 mt-3">
                           <div class="flex items-center gap-1.5">
                             <span class="w-1.5 h-1.5 rounded-full bg-primary-500"></span>
                             <span class="text-[9px] font-black text-primary-400 uppercase tracking-widest">{session.records.filter((r: any) => r.status === 'P').length} Present</span>
                           </div>
                           <div class="flex items-center gap-1.5">
                             <span class="w-1.5 h-1.5 rounded-full bg-yellow-500"></span>
                             <span class="text-[9px] font-black text-yellow-400 uppercase tracking-widest">{session.records.filter((r: any) => r.status === 'T').length} Late</span>
                           </div>
                           <div class="flex items-center gap-1.5">
                             <span class="w-1.5 h-1.5 rounded-full bg-red-500"></span>
                             <span class="text-[9px] font-black text-red-400 uppercase tracking-widest">{session.records.filter((r: any) => r.status === 'A').length} Absent</span>
                           </div>
                        </div>
                     </div>
                  </div>
                  ...
                  >
                    {showingDate === session.date ? 'Hide Details' : 'View Report'}
                  </button>
               </div>
        ...
        <div class="bg-zinc-900/50 border border-zinc-800 p-8 rounded-[32px] space-y-8 relative overflow-hidden group hover:border-zinc-700 transition-all shadow-xl">
           ...
           <div class="flex items-center gap-4 relative z-10">
              ...
              <div>
                 ...
                 <div class="flex items-center gap-2 mt-1.5">
                   ...
                   <p class="text-[9px] font-black text-primary-400 uppercase tracking-widest">{stats.attendance_rate}% PERFORMANCE</p>
                 </div>
              </div>
           </div>
           ...
           <div class="space-y-4 relative z-10">
              <div class="flex items-baseline justify-between">
                 <span class="text-[9px] font-black text-zinc-500 uppercase tracking-widest leading-none">Global Punctuality</span>
