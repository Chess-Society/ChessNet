...
export const load: PageServerLoad = async ({ locals, params }) => {
  ...
  try {
    ...
    const schoolId = (classData as any).schoolId || (classData as any).schoolId;
    
    // Filter students by owner OR school
    const filteredStudents = allStudentsSnap.docs
      .map((d: any) => ({ id: d.id, ...d.data() }))
      .filter((s: any) => {
        const isOwner = s.ownerId === uid || s.ownerId === uid;
        const inSchool = schoolId && (s.schoolId === schoolId || s.schoolId === schoolId);
        return isOwner || inSchool;
      });

    const enrolledDocs = enrolledSnap.docs.map((d: any) => ({ id: d.id, ...d.data() }));

    const enrolledStudentIds = new Set(enrolledDocs.map((ed: any) => ed.studentId || ed.studentId));
    
    // Students already in this class
    const enrolledStudents = enrolledDocs.map((ed: any) => {
      const sId = ed.studentId || ed.studentId;
      const studentDetail = filteredStudents.find((s: any) => s.id === sId);
      return {
        ...studentDetail,
        ...ed,
        enrolledAt: ed.enrolledAt || ed.enrolledAt
      };
    }).filter((s: any) => s.id);
...
export const actions: Actions = {
  enroll: async ({ request, locals, params }) => {
    ...
    try {
      // Check if already enrolled in this class (avoid duplicates)
