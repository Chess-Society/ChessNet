    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-8">
      <div class="space-y-4 text-center md:text-left">
        ...
        <div class="flex flex-col md:flex-row items-center gap-6">
          ...
          <div>
            <h1 class="text-4xl font-black text-white tracking-tighter font-outfit uppercase">{$t('classes.new_title')}</h1>
            <p class="text-zinc-500 text-sm font-medium font-jakarta">{$t('classes.new_subtitle')}</p>
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center justify-center gap-4 bg-zinc-900/50 p-2 rounded-full border border-zinc-800">
        ...
        >
          ...
          {:else}
            ...
            <span>{$t('classes.create_btn')}</span>
          {/if}
        </button>
      </div>
    </div>
      ...
      <div class="lg:col-span-8 space-y-8">
        <div class="bento-card rounded-24 p-8 space-y-8">
          <div class="flex items-center gap-4 border-b border-zinc-800 pb-6">
            ...
            <h3 class="text-lg font-bold text-white font-outfit">{$t('classes.basic_info')}</h3>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-2">
              <label for="name" class="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1 font-outfit">{$t('classes.name_label')}</label>
            ...
            <div class="space-y-2">
              <label for="school" class="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1 font-outfit">{$t('classes.school_label')}</label>
              {#if isPreSelectedSchool}
                <div class="w-full bg-zinc-900/80 border border-violet-600/30 rounded-2xl px-5 py-4 flex items-center justify-between">
                  ...
                  <span class="text-[8px] font-black text-violet-400 uppercase tracking-widest bg-violet-600/10 px-2 py-1 rounded-md">{$t('classes.linked')}</span>
                </div>
              {:else}
                <div class="space-y-4">
                  <div class="relative group">
                    <select 
                      ...
                      class="w-full bg-zinc-950 border-2 border-zinc-900 rounded-2xl px-6 py-4 text-white focus:border-primary-500 transition-all appearance-none cursor-pointer group-hover:border-zinc-800 text-sm font-bold font-body"
                    >
                      <option value="">{$t('classes.independent')}</option>
                      {#each (schools as any[]) as school}
                        <option value={school.id}>{school.name}</option>
                      {/each}
                    </select>
                    <div class="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none text-zinc-500 group-hover:text-primary-400 transition-colors">
                      <CaretDown weight="bold" class="w-4 h-4" />
                    </div>
                  </div>

                  <div class="p-4 bg-primary-500/5 border border-primary-500/10 rounded-xl flex items-start gap-3">
                    <div class="mt-0.5 text-primary-400">
                      <Info weight="duotone" size={16} />
                    </div>
                    <div>
                      <p class="text-[10px] font-black text-white uppercase tracking-widest mb-1">{$t('classes.independent_info_title')}</p>
                      <p class="text-[10px] text-zinc-500 font-medium leading-relaxed">
                        {$t('classes.independent_info_desc')}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div class="bento-card rounded-24 p-8 overflow-hidden relative group border-2 border-dashed border-zinc-800/50 hover:border-violet-600/30 transition-all" transition:fly={{ y: 20 }}>
            ...
            <div class="space-y-6">
              <div class="flex items-center justify-between">
                 <h3 class="text-[10px] font-black text-violet-400 uppercase tracking-[0.2em] font-outfit">{$t('classes.preview')}</h3>
                 <div class="px-3 py-1 bg-zinc-900 rounded-full border border-zinc-800 text-[8px] font-black text-zinc-500 tracking-widest uppercase">{$t('classes.draft')}</div>
              </div>
              ...
              <div class="flex items-center gap-6">
                <div class="w-20 h-20 rounded-2xl bg-zinc-950 border border-zinc-800 flex items-center justify-center text-violet-400 group-hover:scale-110 transition-transform duration-500">
                   <ChalkboardTeacher weight="duotone" size={40} />
                </div>
                <div class="space-y-1">
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <p class="text-zinc-500 text-[10px] font-bold uppercase tracking-widest font-jakarta">
                      {formData.school_id ? getSchoolName(formData.school_id) : $t('classes.not_assigned')}
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div class="lg:col-span-4 space-y-6">
        <div class="bento-card rounded-24 p-8 space-y-6">
          <div class="flex items-center gap-3">
            ...
            <h3 class="text-[10px] font-black text-white uppercase tracking-[0.2em] font-outfit">{$t('classes.guidance_title')}</h3>
          </div>
          ...
          <div class="space-y-6">
            <div class="space-y-2 group">
              <div class="flex items-center gap-2">
                ...
                <p class="text-[11px] font-black text-zinc-300 uppercase tracking-wider font-outfit">{$t('classes.guidance_clarity_title')}</p>
              </div>
            ...
            <div class="space-y-2 group">
              <div class="flex items-center gap-2">
                <div class="w-1.5 h-1.5 rounded-full bg-violet-500 transition-all group-hover:scale-150"></div>
                <p class="text-[11px] font-black text-zinc-300 uppercase tracking-wider font-outfit">{$t('classes.guidance_sync_title')}</p>
              </div>
              <p class="text-xs text-zinc-500 leading-relaxed font-jakarta pl-3.5">
