<script lang="ts">
  ...
  const difficultyLevels = [
    { value: 1, label: 'Muy Fácil', color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
    { value: 2, label: 'Fácil', color: 'text-green-400', bg: 'bg-green-400/10' },
    { value: 3, label: 'Intermedio', color: 'text-amber-400', bg: 'bg-amber-400/10' },
    { value: 4, label: 'Difícil', color: 'text-orange-400', bg: 'bg-orange-400/10' },
    { value: 5, label: 'Muy Difícil', color: 'text-rose-400', bg: 'bg-rose-400/10' }
  ];
  ...
  const validateForm = () => {
    ...
    if (validObjectives.length === 0) errors.learning_objectives = 'Añade al menos un objetivo';

    const validCriteria = formData.assessment_criteria.filter(c => c.trim());
    if (validCriteria.length === 0) errors.assessment_criteria = 'Añade al menos un criterio';
    ...
    <header class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div class="space-y-1">
        <div class="flex items-center gap-2 text-sm font-medium text-zinc-500 mb-2">
          <a href="/panel/habilidades" class="hover:text-violet-400 transition-colors">Habilidades</a>
          <span>/</span>
          <span class="text-zinc-300">Editar</span>
        </div>
        <div class="flex items-center gap-4">
          <button 
            onclick={() => goto('/panel/habilidades')}
          ...
          <div>
            <h1 class="text-3xl font-bold bg-gradient-to-r from-white to-zinc-500 bg-clip-text text-transparent">
              {formData.name || 'Editar Habilidad'}
            </h1>
            <p class="text-zinc-500">Ajusta los detalles y la progresión de esta habilidad</p>
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div class="lg:col-span-8 space-y-6">
        ...
        <section class="bg-zinc-900/50 border border-zinc-800 rounded-[24px] p-8 space-y-8">
          <div class="flex items-center gap-3 mb-2">
            ...
            <h2 class="text-xl font-bold text-white">Configuración Principal</h2>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-2">
              <label for="skill_name" class="text-sm font-semibold text-zinc-300 ml-1">Nombre de la Habilidad</label>
              <input 
            ...
            <div class="space-y-2">
              <label for="category_id" class="text-sm font-semibold text-zinc-300 ml-1">Categoría</label>
              <select 
            ...
            <div class="md:col-span-2 space-y-2">
              <label for="skill_description" class="text-sm font-semibold text-zinc-300 ml-1 text-white">Descripción del Aprendizaje</label>
              <textarea 
                id="skill_description"                bind:value={formData.description}
                rows="4"
                placeholder="Describe qué habilidades adquirirá el alumno..."
            ...
            <div class="space-y-4">
              <p class="text-sm font-semibold text-zinc-300 ml-1">Nivel de Dificultad</p>
            ...
            <div class="space-y-2">
              <label for="estimated_hours" class="text-sm font-semibold text-zinc-300 ml-1">Inversión de Tiempo (Horas)</label>
              <div class="relative">
                <input 
          ...
          <section class="bg-zinc-900/50 border border-zinc-800 rounded-[24px] p-6 space-y-6">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <h3 class="font-bold text-white">Objetivos</h3>
              </div>
              <button 
          ...
          <section class="bg-zinc-900/50 border border-zinc-800 rounded-[24px] p-6 space-y-6">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <h3 class="font-bold text-white">Criterios</h3>
              </div>
              <button 
          ...
          <section class="bg-zinc-900/50 border border-zinc-800 rounded-[24px] p-6 space-y-6">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <h3 class="font-bold text-white">Recursos Sugeridos</h3>
              </div>
              <button 
        ...
        <section class="bg-zinc-900/50 border border-zinc-800 rounded-[24px] p-8 space-y-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              <h2 class="text-xl font-bold text-white">Dependencias</h2>
              <p class="text-sm text-zinc-500">Habilidades que el alumno debe dominar previamente</p>
            </div>
          </div>
          ...
          {#if availablePrerequisites.length === 0}
            <div class="text-center py-12 bg-zinc-950/50 rounded-3xl border border-zinc-800 border-dashed">
              <p class="text-zinc-500 italic">No hay habilidades previas configuradas aún.</p>
            </div>
          {:else}
      ...
      <aside class="lg:col-span-4 space-y-6">
        ...
        <section class="bg-zinc-900 border border-zinc-800 rounded-[24px] p-6 space-y-6">
          <div class="flex items-center justify-between">
            <h3 class="font-bold text-white">Estado del Contenido</h3>
            <div class="flex items-center gap-2">
              <span class="text-[10px] font-bold uppercase tracking-wider text-zinc-500">¿Activa?</span>
              <button 
                onclick={() => formData.active = !formData.active}
                aria-label="Alternar estado de la habilidad"
          ...
          <div class="grid grid-cols-2 gap-3">
            <div class="bg-zinc-950 p-4 rounded-2xl border border-zinc-800 text-center">
              ...
              <span class="text-[10px] font-bold uppercase text-zinc-500">Objetivos</span>
            </div>
            <div class="bg-zinc-950 p-4 rounded-2xl border border-zinc-800 text-center">
              <span class="block text-2xl font-bold text-white mb-0.5">{formData.assessment_criteria.filter(c => c.trim()).length}</span>
              <span class="text-[10px] font-bold uppercase text-zinc-500">Criterios</span>
            </div>
          </div>

          <div class="pt-4 border-t border-zinc-800 space-y-3">
             <div class="flex items-center justify-between text-xs">
              <span class="text-zinc-500">Progresión Sugerida</span>
              <span class="text-white font-medium">Auto-ordenado</span>
            </div>
            <div class="flex items-center justify-between text-xs">
              <span class="text-zinc-500">Última Modificación</span>
              <span class="text-zinc-400 italic">Hoy</span>
            </div>
          </div>
        </section>
        ...
        {#if showPreview}
          <section in:fade out:fade class="bg-violet-600 border border-violet-500 rounded-[24px] p-8 text-white space-y-6 shadow-2xl shadow-violet-500/40">
            ...
            <div>
              <h3 class="text-2xl font-bold line-clamp-2">{formData.name || 'Nombre de la Skill'}</h3>
              <p class="text-white/80 text-sm mt-3 leading-relaxed line-clamp-4">
                {formData.description || 'Describe tu habilidad para ver cómo luce...'}
              </p>
            </div>
        ...
        <section class="bg-zinc-900 border border-zinc-800 rounded-[24px] p-6 space-y-4">
          ...
          <p class="text-xs text-zinc-500 leading-relaxed italic">
            "Para un aprendizaje efectivo, intenta que cada habilidad no tome más de 3-5 horas de estudio antes de aplicar un criterio de evaluación práctico."
          </p>
        </section>
...
{#if showConfirmDiscard}
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4" transition:fade>
    ...
    <div class="bg-zinc-900 border border-zinc-800 rounded-[32px] p-8 max-w-md w-full relative z-10 shadow-2xl" in:fly={{ y: 20 }}>
      ...
      <h3 class="text-2xl font-bold text-white mb-2">¿Descartar cambios?</h3>
      <p class="text-zinc-400 mb-8 leading-relaxed">
        Perderás todos los ajustes realizados en esta sesión. Esta acción no se puede deshacer.
      </p>
