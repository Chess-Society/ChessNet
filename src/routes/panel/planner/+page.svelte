  ...
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
    ...
    <div class="lg:col-span-4 space-y-6">
      ...
      <div class="bento-card p-6 overflow-hidden group">
        <div class="absolute top-0 right-0 p-6 opacity-5 group-hover:scale-110 transition-transform">
          <Trophy size={80} weight="duotone" class="text-primary-400" />
        </div>
        <h3 class="text-white font-bold mb-2">Next Big Event</h3>
        <p class="text-slate-400 text-xs mb-4">Winter League Final</p>
        <div class="flex items-center gap-3 text-primary-400 text-xs font-bold bg-primary-500/5 py-3 px-4 rounded-xl border border-primary-500/10 mb-4">
    ...
    <div class="lg:col-span-8 space-y-6">
      <div class="flex items-center justify-between px-2">
        <h2 class="text-xl font-bold text-white tracking-tight">Schedule for Today</h2>
        <button class="text-sm font-bold text-violet-400 hover:text-violet-300 transition-colors">View week</button>
      </div>
      ...
      <div class="space-y-4">
            ...
            <div class="flex-1 space-y-2">
              <div class="flex items-center gap-3">
                 <span class="px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border
                   {event.type === 'class' ? 'bg-violet-500/10 border-violet-500/20 text-violet-400' : 'bg-primary-500/10 border-primary-500/20 text-primary-400'}">
                   {event.tag}
                 </span>
                 {#if i === 0}
              ...
              <div class="flex flex-wrap gap-4 text-slate-500 text-xs font-medium">
                 ...
                 <div class="flex items-center gap-1.5">
                    ...
                    {event.type === 'class' ? '12 students' : 'Free'}
                 </div>
              </div>
            </div>
        ...
        {:else}
          <div class="bento-card p-12 text-center space-y-4">
             ...
             <div>
                <h4 class="text-white font-bold">No events</h4>
                <p class="text-slate-500 text-sm">Enjoy your free time or schedule a new session.</p>
             </div>
             <button 
               onclick={() => goto('/panel/tournaments/create')}
      ...
      <div class="bg-violet-600/5 border border-violet-500/10 rounded-[2rem] p-8 flex items-center gap-6 relative overflow-hidden">
        ...
        <div>
           <h4 class="text-white font-bold mb-1">Tip of the day</h4>
           <p class="text-slate-400 text-sm leading-relaxed max-w-md">
             Your mornings are 20% more productive if you organize tournaments on Thursdays. Keep it up!
           </p>
        </div>
      </div>
    </div>
