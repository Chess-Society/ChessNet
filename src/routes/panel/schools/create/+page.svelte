<script lang="ts">
  ...
  async function createSchool() {
    if (!name.trim()) {
      showError(new Error($t('schools.form.name_required')));
      return;
    }
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || $t('schools.create_error'));

      showToast.success($t('schools.create_success'));
      await invalidateAll();
      await new Promise(resolve => setTimeout(resolve, 400));
      await goto(`/panel/schools/${result.school.id}`);
...
<div class="max-w-4xl mx-auto px-6 py-12 space-y-12" in:fade>
  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center gap-6">
      ...
      <div>
        <h1 class="text-4xl font-outfit font-black text-white tracking-tighter uppercase">{$t('schools.new_title')}</h1>
        <p class="text-zinc-500 font-jakarta text-sm">{$t('schools.new_subtitle')}</p>
      </div>
    </div>
  </div>
    ...
    <div class="space-y-6">
      <div class="bg-violet-500/5 border border-violet-500/10 rounded-3xl p-6 space-y-6">
        <div class="flex items-center gap-3">
          ...
          <h3 class="text-xs font-black text-white uppercase tracking-widest">{$t('schools.guide.title')}</h3>
        </div>
        
        <div class="space-y-4">
          {#each [
            { text: $t('schools.guide.step1'), color: 'border-violet-500' },
            { text: $t('schools.guide.step2'), color: 'border-blue-500' },
            { text: $t('schools.guide.step3'), color: 'border-primary-500' },
            { text: $t('schools.guide.step4'), color: 'border-amber-500' }
          ] as step}
      ...
      <div class="bg-zinc-900/30 border border-zinc-800 rounded-3xl p-6 space-y-4">
        <h3 class="text-[10px] font-black text-zinc-500 uppercase tracking-widest">{$t('schools.help.title')}</h3>
