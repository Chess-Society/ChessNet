...
<div class="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-8" transition:fade>
  <form method="POST" action="?/update" use:enhance class="space-y-8 pb-32">
      ...
      <div class="bento-card p-6 sm:p-10">
        ...
        <div class="flex flex-col md:flex-row gap-12 items-start">
          <div class="relative group">
            <div class="w-32 h-32 rounded-none bg-zinc-950 border border-white/10 flex items-center justify-center text-slate-700 overflow-hidden relative shadow-[0_0_30px_rgba(0,0,0,0.5)] transition-all duration-500 group-hover:border-violet-500/50 group-hover:shadow-[0_0_20px_rgba(139,92,246,0.15)]">
              {#if $form.teacherAvatar}
                <img src={$form.teacherAvatar} alt="Avatar" class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700" />
              {:else}
          ...
          <div class="flex-grow space-y-6 w-full">
            <div class="space-y-2">
              <label for="teacherName" class="text-[10px] font-outfit font-black text-slate-500 uppercase tracking-[0.2em] ml-1">
            ...
            <div class="space-y-2">
              <label for="teacherEmail" class="text-[10px] font-outfit font-black text-slate-500 uppercase tracking-[0.2em] ml-1">
    ...
    <!-- Floating Save Bar -->
    <div class="fixed bottom-4 sm:bottom-8 left-0 right-0 flex justify-center z-50 pointer-events-none pb-[env(safe-area-inset-bottom)]">
      <button 
        type="submit"
        disabled={$delayed}
        class="bg-violet-600 hover:bg-violet-500 text-white font-outfit font-black px-8 sm:px-10 py-4 sm:py-5 shadow-violet-flare flex items-center gap-3 transition-all hover:scale-105 active:scale-95 pointer-events-auto text-[10px] sm:text-sm rounded-none disabled:opacity-50"
      >
        {#if $delayed}
