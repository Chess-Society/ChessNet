...
<svelte:head>
  <title>{$t('skills.edit.title')} - ChessNet</title>
</svelte:head>

<form method="POST" action="?/update" use:enhance class="min-h-screen bg-zinc-950 text-white pb-32" in:fade>
  <!-- Premium Top Bar -->
    ...
    <div class="max-w-[1200px] mx-auto px-6 py-4 flex items-center justify-between">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-sm font-black uppercase tracking-widest italic flex items-center gap-2">
            <TrendUp weight="fill" class="w-4 h-4 text-amber-500" />
            {$t('skills.edit.title')}
          </h1>
          <p class="text-[10px] font-bold text-zinc-500 uppercase tracking-tighter">{$t('skills.edit.subtitle')}</p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <button 
          ...
          class="px-4 py-2.5 rounded-none text-zinc-500 hover:text-rose-500 transition-all"
        >
        ...
        <button 
          type="submit"
          disabled={$delayed}
          class="px-8 py-2.5 rounded-none bg-amber-600 text-white shadow-lg shadow-amber-600/20 hover:shadow-amber-600/40 hover:-translate-y-0.5 transition-all text-xs font-black tracking-widest uppercase flex items-center gap-2 disabled:opacity-50"
        >
          {#if $delayed}
          ...
          {/if}
          {$delayed ? $t('common.saving') : $t('common.save')}
        </button>
      </div>
    </div>
  </header>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
      ...
      <div class="lg:col-span-8 space-y-8">
        ...
        <section class="bg-zinc-900/30 border border-zinc-800/50 rounded-none overflow-hidden backdrop-blur-sm shadow-2xl">
          <div class="bg-gradient-to-r from-amber-600/10 to-transparent p-8 border-b border-zinc-800/50">
            <h2 class="text-xl font-black uppercase tracking-widest italic flex items-center gap-3">
              <span class="w-1.5 h-6 bg-amber-500 rounded-none"></span>
          ...
          <div class="p-8 space-y-10">
              ...
              <div class="space-y-6 md:col-span-2">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" role="group" aria-labelledby="category_label">
                  {#each categories as category, i}
                    <button
                      type="button"
                      onclick={() => $form.categoryId = category.id}
                      class="selection-card small {$form.categoryId === category.id ? 'active' : ''}"
                      aria-pressed={$form.categoryId === category.id}
                    >
                      <div class="card-icon">
                        <Tag weight={$form.categoryId === category.id ? "fill" : "duotone"} />
                      </div>
                ...
                {#if $errors.categoryId}
                  <p class="text-rose-400 text-[10px] font-bold uppercase flex items-center gap-1.5 ml-1 mt-2">
                    <WarningCircle weight="fill" class="w-3.5 h-3.5" /> {$errors.categoryId}
                  </p>
                {/if}
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
              <div class="space-y-4">
                ...
                <input type="hidden" name="difficulty" bind:value={$form.difficulty} />
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {#each difficultyLevels as level}
                    <button 
                      type="button"
                      onclick={() => $form.difficulty = level.value}
                      class="selection-card small {$form.difficulty === level.value ? 'active' : ''}"
                    >
                      <div class="card-icon">
                        <level.icon weight={$form.difficulty === level.value ? "fill" : "duotone"} />
                      </div>
                      <div class="card-content">
                      ...
                      {#if $form.difficulty === level.value}
                        <div class="card-check" in:scale>
              ...
              <div class="space-y-4">
                ...
                <input type="hidden" name="estimatedHours" bind:value={$form.estimatedHours} />
                <div class="bg-zinc-950/50 border border-zinc-800 rounded-none p-6 flex items-center justify-between shadow-inner">
                  <button 
                    type="button"
                    onclick={() => $form.estimatedHours = Math.max(0.5, $form.estimatedHours - 0.5)}
                  ...
                  >
                    <Clock weight="bold" class="rotate-180" />
                  </button>
                  <div class="text-center">
                    <span class="text-4xl font-black tabular-nums">{$form.estimatedHours}</span>
                    <span class="text-[10px] font-black text-zinc-600 block uppercase">{$t('common.duration_unit') || 'Hours'}</span>
                  </div>
                  <button 
                    type="button"
                    onclick={() => $form.estimatedHours += 0.5}
        ...
        <section class="bg-zinc-900/30 border border-zinc-800/50 rounded-none p-8 space-y-6 shadow-xl">
           <div class="flex items-center justify-between">
              ...
              <div class="px-3 py-1 bg-amber-500/10 text-amber-500 rounded-none text-[9px] font-black uppercase tracking-widest">
                {$t('skills.ui.dependencies')}: {$form.prerequisites.length}
              </div>
            </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[300px] overflow-y-auto no-scrollbar pr-1">
                {#each availablePrerequisites as skill}
                  <button 
                    ...
                    {$form.prerequisites.includes(skill.id) 
                  ...
                  >
                    <div class="w-10 h-10 rounded-none flex items-center justify-center transition-all
                      {$form.prerequisites.includes(skill.id) ? 'bg-amber-600 text-white shadow-lg shadow-amber-600/20' : 'bg-zinc-900 text-zinc-700 group-hover/req:text-zinc-500'}">
                       {#if $form.prerequisites.includes(skill.id)}
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
           ...
           <section class="bg-zinc-900/30 border border-zinc-800/50 rounded-none p-8 space-y-6 shadow-xl">
              <div class="flex items-center justify-between">
                <h3 class="text-sm font-black uppercase tracking-widest italic flex items-center gap-3">
                  <Check weight="duotone" class="w-5 h-5 text-amber-400" />
                  {$t('skills.create.objectives')}
                </h3>
                <button 
                  type="button"
                  onclick={() => addItem('learningObjectives')}
              ...
              <div class="space-y-4">
                {#each $form.learningObjectives as obj, i}
                  <div class="flex gap-4 group" in:slide={{ duration: 250 }}>
                    <div class="w-10 h-10 rounded-none bg-zinc-950 border border-zinc-800 flex items-center justify-center text-[11px] font-black text-zinc-600 shrink-0 mt-1 shadow-inner group-hover:text-amber-500 group-hover:border-amber-500/30 transition-all">
                      {i + 1}
                    </div>
                    <textarea
                      bind:value={$form.learningObjectives[i]}
                    ...
                    <button 
                      type="button"
                      onclick={() => removeItem('learningObjectives', i)}
           ...
           <section class="bg-zinc-900/30 border border-zinc-800/50 rounded-none p-8 space-y-6 shadow-xl">
              <div class="flex items-center justify-between">
                ...
                <button 
                  type="button"
                  onclick={() => addItem('assessmentCriteria')}
                  class="w-8 h-8 rounded-none bg-amber-600/10 text-amber-400 hover:bg-amber-600 hover:text-white transition-all flex items-center justify-center shadow-lg"
                >
                  <Plus weight="bold" />
                </button>
              ...
              <div class="space-y-4">
                {#each $form.assessmentCriteria as cr, i}
                  <div class="flex gap-4 group" in:slide={{ duration: 250 }}>
                    ...
                    <textarea
                      bind:value={$form.assessmentCriteria[i]}
                    ...
                    <button 
                      type="button"
                      onclick={() => removeItem('assessmentCriteria', i)}
                      class="opacity-0 group-hover:opacity-100 p-2 text-zinc-700 hover:text-rose-400 transition-all active:scale-90"
                    >
                      <Trash weight="bold" />
                    </button>
              ...
              {#each $form.resources as res, i}
                <div class="relative group" in:fade>
                   <div class="absolute left-4 top-1/2 -translate-y-1/2 text-zinc-700 group-focus-within:text-blue-500 transition-colors pointer-events-none">
                    <Star weight="bold" class="w-4 h-4" />
                  </div>
                  <input
                    type="text"
                    bind:value={$form.resources[i]}
      ...
      <aside class="lg:col-span-4 space-y-8 h-fit lg:sticky lg:top-[100px]">
        ...
        <section class="bg-zinc-900/50 border border-zinc-800 rounded-none p-8 space-y-8 shadow-2xl relative overflow-hidden">
           <div class="absolute -top-10 -right-10 w-32 h-32 bg-amber-600/5 blur-3xl rounded-none"></div>
           ...
           <div class="flex flex-col items-center gap-6 relative z-10">
              <div class="relative group">
                ...
                <div class="absolute -bottom-2 -right-2 w-10 h-10 rounded-none bg-gradient-to-br from-amber-600 to-orange-600 border border-white/20 flex items-center justify-center text-white shadow-xl animate-pulse">
                   <Smiley weight="bold" class="w-5 h-5" />
                </div>
              </div>
              
              <div class="text-center space-y-2 w-full">
                <label for="icon" class="text-[10px] font-black text-zinc-600 uppercase tracking-[0.2em]">{$t('skills.ui.avatar')}</label>
                <input
                  id="icon"
                  name="icon"
                  type="text"
                  ...
                  class="w-full bg-zinc-950 border border-zinc-800 rounded-none px-4 py-4 text-center text-2xl focus:border-amber-500 outline-none transition-all shadow-inner font-bold"
                />
              </div>
           </div>
           ...
           <div class="space-y-6 pt-6 border-t border-zinc-800/50 relative z-10 font-bold">
              <div class="flex items-center justify-between">
                <div>
                  ...
                  <p class="text-[10px] {$form.active ? 'text-emerald-500' : 'text-zinc-600'} mt-1 uppercase tracking-tighter italic">
                    {$form.active ? ($t('common.status_active') || 'Public & Teaching') : ($t('common.status_inactive') || 'Draft / Private')}
                  </p>
                </div>
                <input type="hidden" name="active" bind:value={$form.active} />
                <button 
                  type="button"
                  onclick={() => $form.active = !$form.active}
                  aria-label="Toggle active status"
                  class="w-14 h-7 rounded-none relative transition-all duration-300 shadow-xl
                  {$form.active ? 'bg-amber-600 shadow-amber-600/20' : 'bg-zinc-800'}"
                >
                  <div class="absolute top-1 left-1 w-5 h-5 rounded-none bg-white transition-all transform 
                  {$form.active ? 'translate-x-7' : 'translate-x-0'} shadow-md"></div>
                </button>
              </div>
              ...
              <div class="space-y-4">
                 <div class="flex items-center justify-between text-xs">
                  ...
                  <input type="hidden" name="orderIndex" bind:value={$form.orderIndex} />
                  <div class="flex items-center gap-2">
                    <button type="button" onclick={() => $form.orderIndex = Math.max(0, $form.orderIndex - 1)} class="w-8 h-8 rounded-none bg-zinc-950 border border-zinc-800 flex items-center justify-center text-zinc-600 hover:text-white transition-colors">-</button>
                    <span class="w-10 text-center font-black text-amber-400 tabular-nums">{$form.orderIndex}</span>
                    <button type="button" onclick={() => $form.orderIndex += 1} class="w-8 h-8 rounded-none bg-zinc-950 border border-zinc-800 flex items-center justify-center text-zinc-600 hover:text-white transition-colors">+</button>
                  </div>
                </div>
              </div>
           </div>
        ...
        <section in:fade class="bg-gradient-to-br from-zinc-900 to-black border border-white/5 rounded-none p-8 text-white space-y-8 shadow-2xl relative overflow-hidden group">
          <div class="absolute top-0 right-0 w-40 h-40 bg-amber-600/10 blur-[60px] rounded-none transition-all duration-700"></div>
          
          <div class="flex items-center justify-between relative z-10">
            <div class="px-3 py-1.5 bg-amber-600/10 border border-amber-500/20 rounded-none text-[9px] font-black uppercase tracking-widest text-amber-400">{$t('skills.ui.live_preview')}</div>
            <div class="flex items-center gap-1 bg-black/40 px-3 py-1.5 rounded-none border border-white/5">
              {#each [1,2,3,4,5] as star}
                <Star weight={$form.difficulty >= star ? 'fill' : 'regular'} size={11} class={$form.difficulty >= star ? 'text-amber-400' : 'text-zinc-800'} />
              {/each}
            </div>
          </div>
          ...
          <div class="relative z-10 space-y-4">
            ...
            <div>
              <h3 class="text-2xl font-black uppercase tracking-tight line-clamp-2 leading-none">
                {$form.name || ($t('skills.create.topic_name') || 'Set a module name')}
              </h3>
              <p class="text-zinc-500 text-[11px] mt-4 leading-relaxed line-clamp-3 italic font-medium">
                "{$form.description || ($t('skills.create.description_placeholder') || 'Add content detail to see it here...')}"
              </p>
            </div>
          </div>
          ...
          <div class="pt-6 border-t border-white/5 relative z-10 flex items-center justify-between">
            <div class="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-zinc-500 group-hover:text-zinc-300 transition-colors">
              <Clock weight="fill" class="w-4 h-4 text-amber-500" /> {$form.estimatedHours} {$t('common.duration_unit') || 'Hours'}
            </div>
...
{#if showDeleteConfirm}
  <div class="fixed inset-0 z-[100] flex items-center justify-center p-6" in:fade>
    <button 
      class="absolute inset-0 bg-zinc-950/90 backdrop-blur-md cursor-default"
      onclick={() => showDeleteConfirm = false}
      aria-label="Close"
    ></button>
    <div 
      class="relative w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-none p-10 shadow-2xl space-y-8"
      in:scale={{ start: 0.9, duration: 400 }}
    >
      <div class="w-20 h-20 bg-rose-500/10 text-rose-500 flex items-center justify-center rounded-none mx-auto border border-rose-500/20">
        <Trash weight="fill" class="w-10 h-10" />
      </div>
      
      <div class="text-center space-y-3">
        <h3 class="text-xl font-black uppercase tracking-tighter italic">{$t('common.confirm_delete')}</h3>
        <p class="text-xs text-zinc-500 font-bold uppercase tracking-widest">
          {$t('skills.ui.delete_warning')}
        </p>
      </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <button 
          onclick={() => showDeleteConfirm = false}
          class="py-4 bg-zinc-800 text-white text-[10px] font-black uppercase tracking-widest hover:bg-zinc-700 transition-all rounded-none"
        >
          {$t('common.cancel')}
        </button>
        <form method="POST" action="?/delete">
          <button 
            type="submit"
            class="w-full py-4 bg-rose-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-rose-500 transition-all rounded-none shadow-lg shadow-rose-600/20"
          >
            {$t('common.delete')}
          </button>
        </form>
...
<style lang="postcss">
  ...
  .glass-input {
    @apply w-full bg-zinc-950/50 border border-zinc-800/50 rounded-none px-5 py-4 text-sm text-white focus:border-amber-500/50 focus:bg-zinc-900/50 outline-none transition-all placeholder:text-zinc-700 font-medium shadow-inner;
  }

  .glass-label {
    @apply text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] ml-1 block;
  }

  .selection-card {
    @apply relative flex flex-col gap-3 p-5 bg-zinc-950/30 border border-zinc-800/50 rounded-none text-left transition-all hover:bg-zinc-900/50 hover:border-zinc-700;
  }

  .selection-card.small {
    @apply p-4 flex-row items-center gap-4;
  }

  .selection-card.active {
    @apply bg-amber-600/10 border-amber-500/50 ring-1 ring-amber-500/20 shadow-lg shadow-amber-600/5;
  }

  .card-icon {
    @apply w-10 h-10 flex items-center justify-center bg-zinc-900 text-zinc-500 border border-zinc-800 rounded-none transition-all;
  }

  .selection-card.active .card-icon {
    @apply bg-amber-600 text-white border-amber-400 shadow-lg shadow-amber-600/20;
  }

  .card-title {
    @apply text-[11px] font-black uppercase tracking-widest text-zinc-400 transition-colors;
  }

  .selection-card.active .card-title {
    @apply text-white;
  }

  .card-check {
    @apply absolute top-3 right-3 w-5 h-5 bg-amber-600 text-white rounded-none flex items-center justify-center shadow-lg;
  }

  .selection-card.small .card-check {
