...
import { marked } from 'marked';
  ...
  async function refreshPerformanceReport() {
    ...
    try {
      ...
      if (res.ok) {
        performanceReport = result.analysis;
      } else {
        reportError = result.error || 'Error al generar el análisis';
      }
    } catch (e) {
      reportError = 'Error de conexión con el servidor';
    } finally {
...
<div class="page-container" in:fade>
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-8">
      ...
      <!-- Performance Report Section (Premium) -->
      {#if data?.userPlan === 'premium' || data?.user?.isAdmin}
      <section class="bento-card !p-0 overflow-hidden border-2 border-violet-500/20 relative group/performance">
        ...
        <div class="p-8 md:p-10 relative z-10">
          <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-white/5 pb-8 mb-8">
            <div class="flex items-center gap-4">
              <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-violet-500 to-fuchsia-600 flex items-center justify-center text-white shadow-lg shadow-violet-500/20 group-hover/performance:scale-110 transition-transform">
            ...
            <button 
              onclick={refreshPerformanceReport}
              disabled={isGeneratingReport || !student?.lichessUsername}
          ...
          {#if performanceReport}
            <div class="report-response-container bg-zinc-950/50 rounded-2xl border border-white/5 p-8 md:p-10" in:fade>
              <div class="prose prose-invert prose-violet max-w-none font-jakarta text-slate-300 leading-relaxed">
                {@html marked.parse(performanceReport)}
              </div>
            </div>
