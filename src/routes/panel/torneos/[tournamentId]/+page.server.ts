...
export const load: PageServerLoad = async ({ params, locals }) => {
  ...
  try {
    // Obtener datos del torneo, participantes y emparejamientos en paralelo
    ...
    // Formatear participantes para la UI (registeredPlayers)
    const registeredPlayers = participants.map(p => ({
      ...
      status: 'confirmed', // Firebase simplificado suele ser confirmado directamente
      notes: null
    }));

    // Determinar rondas únicas a partir de los emparejamientos
    ...
    // Formatear emparejamientos para la UI
    ...
    // Generar clasificación (standings) a partir de los participantes
    const standings = participants
      ...
      .map((p, index) => ({
        ...
        performance: p.rating || 1200 // Simplificado
      }));

    // Estudiantes disponibles para inscribir (que no estén ya registrados)
